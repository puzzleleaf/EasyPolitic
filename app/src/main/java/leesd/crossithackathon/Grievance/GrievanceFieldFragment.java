...
public class GrievanceFieldFragment extends Fragment {
    ...
    public final static String[] fields = new String[10];             //칼럼에 들어갈 숫자(1..10)
    public final static int[] fieldNumbers = new int[10];             //칼럼에 들어갈 10대 분야 건수
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, Bundle savedInstanceState) {
        ...
        hashMap = gf.rank(getArguments().getInt("year"));

        //칼럼에 들어갈 Field 데이터 세팅
        for(int i = 0, j = 1 ; i < 10 ; i++, j++){
            fields[i] = "" + j;
            fieldNumbers[i] = (int) hashMap.get("TOP10_NUMBER" + j);
        }

        columnChart = (ColumnChartView) view.findViewById(R.id.chart_bottom);
        generateColumnData();
        prepareDataAnimation();
        columnChart.startDataAnimation(2000);
